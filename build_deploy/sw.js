...
class V {
  ...
  async cachePut(e, t) {
    ...
    if (!r)
      return !1;
    const { cacheName: c, matchOptions: i } = this._strategy, o = await self.caches.open(c), h = this.hasCallback("cacheDidUpdate"), g = h ? await M(
    ...
    try {
      ...
      if (u instanceof Error)
        throw u.name === "QuotaExceededError" && await S(), u;
    }
...
function se(a) {
  const e = L(), t = new te(e, a);
  N(t);
}
const ae = "-precache-", ne = async (a, e = ae) => {
...
async function he() {
  try {
    re(), console.log("Outdated caches have been cleaned up successfully.");
  } catch (a) {
    console.error("Failed to clean outdated caches:", a);
  }
}
he();
...
async function x(a) {
  try {
    let e = "Default Push Message";
    a.data && (e = a.data.text());
    let t = JSON.parse(e);
    const s = {
      body: t.body,
      icon: t.icon,
    ...
    console.error("Failed to handle push event:", e), "sync" in self.registration && await self.registration.sync.register("pushRetry");
  }
}
self.addEventListener("push", async function(a) {
  a.waitUntil(
    x(a)
  );
});
self.addEventListener("sync", function(a) {
  a.tag === "pushRetry" && a.waitUntil(x(a));
});
self.addEventListener("notificationclick", function(a) {
  a.notification.close();
  const e = a.notification.data.url;
  console.log("🚀 ~ file: sw.js:75 ~ event.notification:", a.notification), a.waitUntil(
